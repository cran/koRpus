...
All results of both \texttt{treetag()} and \texttt{tokenize()} have also a slot called \texttt{desc}, which provides various descriptive statistics calculated from the analyzed text. If you deal with this slot for the first time, it's a good idea to first look at its structure:
