...
#
# @slot param Relevant parameters of the given analysis, as given to the function call ("segment", "factor.size", "rand.sample", "case.sens", "lemmatize").
# @slot tt The analyzed text in tokenized form, with four elements ("tokens", "types", "num.tokens", "num.types").
...
setClass("kRp.TTR",
		representation=representation(
			...
			MTLD.char="matrix"),
		prototype(
			param=list(segment=NA, factor.size=NA, case.sens=NA, lemmatize=NA),
			tt=list(tokens=NA, types=NA, num.tokens=numeric(), num.types=numeric()),
			TTR=numeric(),
			MSTTR=list(MSTTR=NA, TTR.seg=NA, dropped=NA),
...
setValidity("kRp.TTR", function(object){
		...
		if(!identical(param.names, c("segment", "factor.size", "case.sens", "lemmatize"))){
			stop(simpleError("Invalid object: Wrong names (slot \"param\")."))
		} else {}

		if(!identical(tt.names, c("tokens", "types", "num.tokens", "num.types"))){
		...
		if(!identical(MSTTR.names, c("MSTTR", "TTR.seg", "dropped")) |
			...
			stop(simpleError("Invalid object: Wrong names (slot \"MSTTR\" or \"MTLD\")."))
		} else {}
