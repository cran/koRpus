...
must be identified an named. That is, it has to be split into its semantic parts (tokens), words, numbers, punctuation marks. After that, each token will
...
The first argument (file name) and \texttt{lang} should explain themselves. The \texttt{treetagger} option can either take the full path to one of the original TreeTagger scripts mentioned above, or the keyword "manual", which will cause the interpretation of what is defined by \texttt{TT.options}. To use a preset, just put the \texttt{path} to your local TreeTagger installation and a valid \texttt{preset} name here.\footnote{As of 0.04-18, English, Italian, German, Spanish and Russian are implemented, refer to package documentation.}

The resulting S4 object is of a class called \texttt{kRp.tagged}. The main results are to be found in the slot \texttt{TT.res}. In addition to TreeTagger's original output (token, tag and lemma) \texttt{treetag()} also automatically counts letters, assigns tokens to global word classes and explains the rather cryptic POS tags:
...
\subsubsection{The \texttt{desc} Slot}

All results of both \texttt{treetag()} and \texttt{tokenize()} have also a slot called \texttt{desc}, which provies various descriptive statistics calculated from the analyzed text. If you deal with this slot for the first time, it's a good idea to first look at its structure:

\begin{Schunk}
	\begin{Sinput}
> str(tagged.text@desc)
	\end{Sinput}
% 	\begin{Soutput}
% 	\end{Soutput}
\end{Schunk}

Amonst others, you will find several indices describing the number of characters:
\begin{description}
...
\begin{Schunk}
	\begin{Sinput}
> freq.analysis.res <- kRp.freq.analysis(tagged.text, corp.freq=LCC.en)
	\end{Sinput}
% 	\begin{Soutput}
% 	\end{Soutput}
\end{Schunk}

The resulting object holds a lot of information, even if no corpus data was used (i.\,e., \texttt{corp.freq=NULL}). To begin with, it contains the two slots \texttt{TT.res} and \texttt{lang}, which are copied from the analyzed tagged text object. In this way analysis results can always be converted back into \texttt{kRp.tagged} objects.\footnote{This can easily be done by calling  \texttt{as(freq.analysis.res, "kRp.tagged")}.} However, if corpus data was provided, the slot \texttt{TT.res} gained three new columns:

\begin{Schunk}
	\begin{Sinput}
> freq.analysis.res@TT.res
	\end{Sinput}
	\begin{Soutput}
...
Amongst others, \texttt{desc} now also gives easy access to character vectors with all words (\texttt{@desc\$all.words}) and all lemmata (\texttt{@desc\$all.lemmata}), all tokens sorted into word classes (e.\,g., all verbs in \texttt{@desc\$classes\$verb})\footnote{This sorting depends on proper POS-tagging, so this will only contain useful data if you used \texttt{treetag()} instead of \texttt{tokenize()}.}, or the number of words in each sentece:

\begin{Schunk}
	\begin{Sinput}
> freq.analysis.res@desc[["sentc.length"]]
	\end{Sinput}
 	\begin{Soutput}
...
As a practical example, the list \texttt{@desc\$classes} has proven to be very helpful to debug the results of TreeTagger, which is remarkably accurate, but of course not free from making a mistake now and then. By looking through \texttt{@desc\$classes}, where all tokens are grouped regarding to the global word class TreeTagger attributed to it, at least obvious errors (like names mistakenly taken for a pronoun) are easily found:\footnote{And can then be corrected by using the function \texttt{correct.tag()}}

\begin{Schunk}
	\begin{Sinput}
> freq.analysis.res@desc$classes
	\end{Sinput}
 	\begin{Soutput}
$conjunction
